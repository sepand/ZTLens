...
The [Zero Trust Matrix](https://sepand.github.io/ZTLens/zero-trust-matrix) page has a Vendors toggle showing vendors identified in each pillar's marketplace category. `data/vendors.yaml` is auto-generated by [`.github/workflows/sync-vendors.yml`](.github/workflows/sync-vendors.yml), which runs on the 1st and 15th of each month (or on demand) and pulls from the **AWS Marketplace Discovery API**. To enable it, add these repo secrets (Settings → Secrets and variables → Actions):

- `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` — an IAM user/role with the `AWSMarketplaceDiscoveryFullAccess` policy (self-serve, no approval needed)
- `AZURE_MARKETPLACE_API_KEY` — optional; requires requesting an API key by email from Microsoft's Marketplace Catalog team. Skipped gracefully if unset.
